          ...
          form_script = my_widget_script =
{
  init:function (mode, json_data) {
	...
	// Populates the drug stock units select upon form initialization
    for (var i = 0; i < my_widget_script.drug_units.length; i++) {
      $('select.drug-stock-units').append('<option value="' + i + '">' + my_widget_script.drug_units[i]["1"] + '</option>');
    }
	
	this.parent_class.init('edit', json_data);
    ...
    if (my_widget_script.is_valid()) {
      ...
      $('.drug-stock-units').each(function() {		  
		...
		var final_units_select = stock_units_select.closest('tr').find('select.drug-final-units');
	  
		// Populate the final units with the appropriate choices based on the stock unit selection
		my_widget_script.populate_final_units(stock_units_select, final_units_select);
	  })
  ...
  volume_format:function (volume) {
	  if (!isNaN(volume)){
		  if (volume >= 1) {
			  ...
			  return parseFloat((volume*1000).toFixed(1)) + " &micro;l";
		  } 
	  } else {
		  return "";
		}
  ...
  drug_units:[
    //{units multiplier:stock unit, units multiplier:acceptable final units}
  	{1:"g/ml", 1000:"mg/ml", 1000000:"&micro;g/ml", 1000000000:"ng/ml"},
	{1:"mg/ml", 1000:"&micro;g/ml", 1000000:"ng/ml"},
	{1:"&micro;g/ml", 1000:"ng/ml"},
	{1:"ng/ml"},
	{1:"M", 1000:"mM", 1000000:"&micro;M", 1000000000:"nM"},
	{1:"mM", 1000:"&micro;M", 1000000:"nM"},
	{1:"&micro;M", 1000:"nM"},
	{1:"nM"}
  ],
  ...
  populate_final_units:function(drug_stock_units, drug_final_units) {
	...
	for (key in my_widget_script.drug_units[stock_units]) {
		drug_final_units.append('<option value="' + key + '">' + my_widget_script.drug_units[stock_units][key] + '</option>');
	}
	
	// If data already set from initial data entry, then keep the same information
	var final_units_hidden = drug_final_units.closest('tr').find('.drug-final-hidden');
  ...
  cell_calculations:function (concentration) {
	  $(".cell-calculation").each(function() {
			...
			if (!isNaN(num_aliquots) || $(this).hasClass("drug-calculation")) {
				my_widget_script.display_result(volume_cells, $(this).find(".volume-cells"));
				my_widget_script.display_result(buffer_volume, $(this).find(".buffer-volume"));
			} else {
				my_widget_script.display_result(NaN, $(this).find(".volume-cells"), false);
				my_widget_script.display_result(NaN, $(this).find(".buffer-volume"), false);
			}	
			...
			my_widget_script.display_result(rCD4_needed, $(this).find("#rcd4-needed"), false);	
		});
  },
  ...
  freeze_calculations:function(concentration){
	$(".freeze-calculation").each(function() {
		...
		my_widget_script.display_result(dmso_volume, $(this).find(".dmso-volume"));
		my_widget_script.display_result(num_tubes, $(this).find(".num-tubes"), false); 
	});
  },
  ...
  drug_calculation:function () {
	  $(".drug-calculation").each(function() {
		  ...
		  if (isNaN(media_volume)){
			media_volume =  $(this).prev(".drug-calculation:has(.media-volume)").find(".media-volume").val();
			
			if (isNaN(media_volume)){
				var curr = $(this).prev(".drug-calculation");
				media_volume =  curr.prev(".drug-calculation:has(.media-volume)").find(".media-volume").val();
			}
		  }
		  ...
		  var drug_stock_multiplier = $(this).find(".drug-final-units").is('span') ? 1 :
			parseFloat($(this).find(".drug-final-units").val());

		  var drug_volume = (media_volume * drug_final) / (drug_stock * drug_stock_multiplier);
		  
		  my_widget_script.display_result(drug_volume, $(this).find(".drug-volume"));
	  });
  },
  ...
  cell_isolation:function(){
	...
	my_widget_script.display_result(sysmex_final_concentration, $("#sysmex-final-concentration"), false);
	my_widget_script.display_result(sysmex_total_cells, $("#sysmex-total-cells"), false);
	...
	var manual_cell_concentration = my_widget_script.manual_count_average(counts) * 10000 * manual_dilution / 1000000;
	// Concentration * volume cells resuspended in
	var manual_total_cells = manual_cell_concentration * volume_resuspended;
	
	my_widget_script.display_result(manual_cell_concentration, $("#manual-final-concentration"), false);
	my_widget_script.display_result(manual_total_cells, $("#manual-total-cells"), false);

	// Average Sysmex and Manual totals
	var average_cell_concentration = (sysmex_final_concentration + manual_cell_concentration) / 2
	...
	my_widget_script.display_result(average_cell_concentration, $("#average-cell-concentration"), false);
	...
	$(".cell-calculation, .freeze-calculation").each(function() {
		...
		if (!isNaN(cells_needed)){
			...
			} else if ($(this).hasClass("freeze-calculation") || $(this).hasClass("drug-calculation")){
	...
	my_widget_script.display_result(remaining_cells, $("#remaining-cells"), false);
...
$(document).ready(function(){
  ...
  $('.drug-final-units').on('input', function() {
	  ...
	  var final_units_hidden = final_units_select.closest('tr').find('.drug-final-hidden');
  ...
  $('.input-change').on('input', function() {my_widget_script.cell_isolation();})
});
